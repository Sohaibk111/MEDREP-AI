...
`;
    ...
    if (qLower.includes('unsupported') || qLower.includes('not verified') || qLower.includes('unknown spec')) {
      ...
      text = `[FACT] EvoCheck vs FreeStyle Libre 1: EvoCheck provides 15-day wear (vs Libre's 14 days), 8.66% MARD (vs Libre's 11.4%), direct continuous Bluetooth Low Energy telemetry every 1 minute without manual NFC scanning (vs Libre's manual NFC scan requirement), and IP68 water resistance (vs Libre's IP27).\n\n[RECOMMENDATION] Position EvoCheck's continuous automated telemetry and 15-day duration as key clinical differentiators for active patient monitoring.`;
